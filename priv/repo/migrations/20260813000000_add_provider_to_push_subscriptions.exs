defmodule Grappa.Repo.Migrations.AddProviderToPushSubscriptions do
  ...
  unlike `:subject`'s DB-enforced XOR, `provider`'s closed set
  (`"webpush"` | `"unifiedpush"`) is validated ONLY at the
  `Ecto.Changeset` layer (`validate_inclusion/3` in
  `Grappa.Push.Subscription.changeset/2`) — every insert/update in
  this codebase already routes through that changeset, and adding a
  DB CHECK here would cost the recreate dance for a guard the
  application layer already gives.
