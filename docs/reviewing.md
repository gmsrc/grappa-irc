...
| cicchetto/ | `cicchetto/src/**` + `cicchetto/{tsconfig.json,vite.config.ts,vitest.config.ts,biome.json,package.json,index.html}` + `cicchetto/public/{manifest.json,sw.js,icon*}`. **`cicchetto/src/themes/*.css` is IN scope and must be READ, not merely globbed** — it is by a wide margin the largest file under `cicchetto/`, it has no automated coverage by construction (the browser is its only oracle), and it grew unwatched for a month because `src/**` contains it while naming nothing an agent would open. See the CSS lens below. |
...
- **CSS** (`cicchetto/src/themes/*.css`) — a first-class product
  surface, not decoration. It carries no automated coverage by
  construction, so this read is the only gate it has. Look for: rules
