...
| cicchetto/ | `cicchetto/src/**` + `cicchetto/{tsconfig.json,vite.config.ts,vitest.config.ts,biome.json,package.json,index.html}` + `cicchetto/public/{manifest.json,sw.js,icon*}`. **`cicchetto/src/themes/*.css` is IN scope and must be READ, not merely globbed** — it is by a wide margin the largest file under `cicchetto/`, its RENDERING has no automated oracle (only a browser can judge it), and it grew unwatched for a month because `src/**` contains it while naming nothing an agent would open. It is not unguarded: 28 test files plus `__tests__/helpers/themeCss.ts` assert source-level invariants over its TEXT. Those catch a rule that vanished or changed shape; none of them can see that it looks wrong. See the CSS lens below. |
...
- **XSS / token leakage / CSP-incompatible patterns** —
  ...
  objects, third-party script loads that the CSP rejects
  (`GrappaWeb.Plugs.SecurityHeaders`, BEAM-emitted since #485).
...
- **PWA shell correctness** — `manifest.json` has all required
  ...
  (`Cache-Control: no-cache` emitted by the BEAM for
  `/service-worker.js` OR an explicit version-bump in the SW
  filename), `index.html` includes the manifest link.
- **CSS** (`cicchetto/src/themes/*.css`) — a first-class product
  surface, not decoration. Source-level invariants ARE tested (see
  `__tests__/helpers/themeCss.ts` and its 28 callers), but nothing
  automated judges how the result LOOKS, so this read is the only
  gate on everything the text-level assertions cannot express. Look
  for: rules
  for one surface scattered across distant bands of the file
  (locality — it is what lets sibling instances drift apart
  unnoticed); per-instance overrides re-declaring the same escape from
  a shared base, where one shared class belongs; controls below the
  iOS 44pt tap-target minimum; hardcoded colours where a theme var
  exists (the #75 gallery layers inline vars over these blocks, so a
  literal silently wins over a user's theme); `!important` (there are
  2 in the whole file — treat any new one as a finding); dead
  selectors matching no markup; base rules whose semantics every
  consumer must override to be usable.
- **Build + tooling drift** — `tsconfig.json` strict mode flags
  ...
  config's base/root/outDir match what the BEAM serves from
  `CIC_DIST_ROOT` (`Plug.Static` + `Grappa.Cic.Bundle`);
  `vite-plugin-solid` is the plugin layer (not raw Babel
  preset shenanigans).
