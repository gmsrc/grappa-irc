...
**Don't want to build?** Every `vX.Y.Z` release also publishes a self-contained, multi-arch image (`linux/amd64` + `linux/arm64`) to **`ghcr.io/vjt/grappa`** (`:<tag>` + `:latest`) — pull it instead of compiling from source. It bundles the ERTS + the cicchetto SPA and boots on its own. Note it is a *release* image: it has **no `Phoenix.CodeReloader`**, so it is not the hot-edit dev environment — the Docker Compose stack above stays the development runtime. See [`INSTALL.md`](INSTALL.md) and `docs/OPERATIONS.md`.
