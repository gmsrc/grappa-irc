...
| cicchetto/ | `cicchetto/src/**`, `cicchetto/{tsconfig.json,vite.config.ts,vitest.config.ts,biome.json,package.json,index.html}`, `cicchetto/public/{manifest.json,sw.js,icon*}`. **`cicchetto/src/themes/*.css` is IN scope and must be read** — `default.css` is by a wide margin the largest file under `cicchetto/` and was materially unreviewed for most of this skill's history: it fell inside the `src/**` glob while appearing nowhere in the checklist below. See the CSS lens in the cicchetto agent's list. |
