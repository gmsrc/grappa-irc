...
describe("perform REST client — #189", () => {
  it("getPerform GETs /networks/:slug/perform and returns the view", async () => {
    const spy = mockFetch({ perform_list: "MODE me +x", oper_pass_set: true });

    const view = await getPerform(TOKEN, SLUG);
    expect(view).toEqual({ perform_list: "MODE me +x", oper_pass_set: true });
  ...
  it("putPerform PUTs the given body verbatim (list + oper_pass)", async () => {
    const spy = mockFetch({ perform_list: "OPER me $oper_pass", oper_pass_set: true });
  ...
  it("putPerform omits oper_pass when the caller does not pass it (leave-blank-to-keep)", async () => {
    const spy = mockFetch({ perform_list: "MODE me +x", oper_pass_set: true });
  ...
  it("URL-encodes the network slug", async () => {
    const spy = mockFetch({ perform_list: null, oper_pass_set: false });
