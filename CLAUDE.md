...
├── Grappa.Identd.Bindings             (227 identd {source,lport,peer,pport} → ident table; OFF unless the operator enables identd)
├── Task.Supervisor                    (name: Grappa.Identd.Acceptors — the identd acceptor pool)
├── Grappa.Identd.Listener             (227 RFC 1413 listener; admin-configured port, never 113 — all three before Endpoint: a session binds on connect and the earliest connect door is the REST one)
